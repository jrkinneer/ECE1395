...
cost_array = []
iteration_array = []
theta = gradientDescent(X_train, y_train, .3, 500, cost_array, iteration_array)
...
total_cost = 0
for k in range(len(X_test)):
    y_pred = (X_test[k][0]*theta_normal[0]) + (X_test[k][1]*theta_normal[1])
    cost_i = (y_pred - y_test[k])**2
    total_cost += cost_i
print("total cost after testing normalEqn: ", total_cost/(2*len(X_test)))
